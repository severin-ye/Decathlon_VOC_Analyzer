...
单商品工作流由 LangGraph 编排为四个节点：\texttt{overview}、\texttt{normalize}、\texttt{index} 和 \texttt{analyze}。\texttt{overview} 统计数据集规模与缺失文件；\texttt{normalize} 将目标商品转化为标准化证据包；\texttt{index} 为商品文本、图片和区域建立索引；\texttt{analyze} 执行评论抽取、问题生成、检索、重排、聚合、报告生成和产物落盘。该流程保证系统既可以通过 API 交互调用，也可以通过批处理脚本复现实验。

\subsection{商品证据标准化}

数据标准化模块以 \texttt{ProductEvidencePackage} 为核心对象。系统从每个商品目录读取 \texttt{product.json}、\texttt{reviews.json} 和 \texttt{images/}，将商品标题、描述和类目转化为 \texttt{TextEvidence}，将图片转化为 \texttt{ImageEvidence}，将评论转化为 \texttt{ReviewRecord}。每个证据对象都具有稳定 ID、来源字段和语言提示。

为了为局部视觉证据预留空间，系统对有效图片生成五类默认区域：\texttt{center\_focus}、\texttt{upper\_focus}、\texttt{lower\_focus}、\texttt{left\_focus} 和 \texttt{right\_focus}。这些区域由相对比例转换为像素框，并被建模为 \texttt{ImageRegion}。当前区域并不等价于语义检测结果，但它使系统能够在没有人工标注框的情况下初步支持区域裁剪、局部 embedding 和图像重排。

标准化过程还会输出数据质量信息，包括商品数量、评论数量、图片数量、缺失 \texttt{product.json} 或 \texttt{reviews.json} 的商品，以及空评论数量。标准化产物写入 \texttt{02\_outputs/1\_normalized/}，后续所有模块优先复用该产物。

\subsection{评论建模与抽样}

评论建模模块将原始评论转化为 \texttt{ReviewAspect}。每个方面对象包含 \texttt{aspect}、\texttt{sentiment}、\texttt{opinion}、\texttt{evidence\_span}、\texttt{usage\_scene}、\texttt{confidence} 和 \texttt{extraction\_mode}。系统首先清洗评论文本，过滤空评论、过短评论和低信息短评，例如 \texttt{ok}、\texttt{good}、\texttt{great} 等。

当设置 \texttt{max\_reviews} 时，系统启用星级抽样。默认 profile 为 \texttt{problem\_first}，其目标比例为 1 星 30\%、2 星 25\%、3 星 20\%、4 星 15\%、5 星 10\%。如果某个星级样本不足，系统按配置的 fallback 顺序补位，并将目标数量、可用数量、实际选择数量、短缺数量和补位数量写入 \texttt{ReviewSamplingPlan}。

方面抽取支持 LLM 和 heuristic 两条路径。LLM 路径使用 Qwen 网关和结构化输出 schema；heuristic 路径使用关键词、评分和负向提示词识别容量、便携性、可用性、耐用性、价值价格、材料硬度等方面。抽取后，系统按评论 ID、方面、情感和证据片段去重，并将结果写入 \texttt{02\_outputs/2\_aspects/}。

\subsection{问题规划}

问题规划模块是本文方法的关键桥接层。系统不直接用评论方面或评论原句检索商品证据，而是先为每个 \texttt{ReviewAspect} 规划若干 \texttt{QuestionIntent}，再生成 \texttt{RetrievalQuestion}。默认意图包括：\texttt{explicit\_support}、\texttt{visual\_confirmation}、\texttt{cross\_modal\_resolution}、\texttt{spec\_check} 和 \texttt{visual\_detail}。

每个问题都包含来源评论、来源方面、自然语言问题、生成理由、期望证据路线和置信度。\texttt{expected\_evidence\_routes} 明确指定检索应使用 \texttt{text}、\texttt{image} 或二者结合。该字段使后续召回具备 route-aware 能力，也使报告归因能够区分文本支持、图像支持和混合支持。
...
索引模块将商品证据转化为 \texttt{IndexedEvidence}。文本证据 route 为 \texttt{text}，图像整图和区域 route 为 \texttt{image}。文本 embedding 可使用 \texttt{text-embedding-v4} API 或本地 Qwen3 embedding；图像 embedding 默认使用 \texttt{openai/clip-vit-base-patch32}。当外部能力不可用且运行策略允许降级时，系统可退回 hash embedding 或代理文本 embedding。

索引后端通过 \texttt{IndexBackend} 抽象统一管理。\texttt{LocalIndexBackend} 将索引快照写入本地 JSON 文件，并在搜索时计算 query 向量与 evidence 向量的相似度；\texttt{QdrantIndexBackend} 为文本和图像分别创建 collection，并用 \texttt{product\_id}、\texttt{category\_slug} 和 \texttt{route} 过滤候选。
...
\subsection{重排、缓存与运行策略}

重排模块将文本候选和图像候选分开处理。文本候选支持本地 Qwen3 reranker、DashScope \texttt{gte-rerank-v2} API 和启发式排序；图像候选支持本地 Qwen3-VL、多模态 Qwen-VL API、文本 reranker 退路和启发式排序。

在多模态图像重排中，系统会读取商品图片，若候选为区域证据则按 \texttt{region\_box} 裁剪，然后压缩为 JPEG data URL 输入 Qwen-VL，并要求模型返回严格 JSON 评分。这使图像候选的最终排序能够基于真实视觉内容，而不是只依赖图片路径或代理文本。

系统将 query embedding 和 rerank 结果写入磁盘缓存。缓存签名绑定 route、query、backend、model、base URL 和候选 digest，因此相同配置可以复用结果，不同模型或候选集不会串写。运行策略由 \texttt{RuntimeExecutionPolicy} 控制：开发时可允许降级，正式实验时可禁止静默降级，满血模式下还会检查 Qdrant、本地 embedding、本地 reranker 和本地多模态 reranker 等前置条件。

\subsection{报告生成、归因与回放}
...
报告生成后，\texttt{ReportRefinementService} 会对洞察和建议去重，依据支持证据类型重新校准 owner 与 evidence level，并修正``声称有图像支持但实际未检索到图像证据''等不一致表述。随后系统构建 \texttt{claim\_attributions}，将报告主张映射到评论、商品文本、商品图片或区域证据，并标记 supported、partial、unsupported 或 contradicted。
...
\subsection{Schema 与可解释性}

本文系统的可解释性来自结构化中间表示。\texttt{schemas/} 目录定义了数据、评论、索引、分析、问题缓存和检索缓存对象。最终 \texttt{ProductAnalysisResponse} 同时包含 extraction、question intents、questions、retrievals、retrieval quality、retrieval runtime、aggregates、report、trace、replay summary、artifact bundle 和 warnings。
...
API 入口由 FastAPI 提供，批处理入口由 \texttt{04\_scripts/run\_workflow.py} 提供，单商品工作流由 LangGraph 编排。LLM 调用通过 OpenAI 兼容 Qwen 网关完成；检索层支持本地 JSON 索引和 Qdrant 索引；图像 embedding 默认使用 CLIP；图像候选重排默认可使用 Qwen-VL。

\subsection{数据与产物}

原始商品数据位于 \texttt{01\_data/01\_raw\_products/products/}，人工审核用中文数据可导出到 \texttt{01\_data/02\_audit\_zh\_products/products/}。系统产物按阶段写入 \texttt{02\_outputs/}：标准化证据包写入 \texttt{1\_normalized/}，评论方面写入 \texttt{2\_aspects/}，索引与缓存写入 \texttt{3\_indexes/}，分析报告写入 \texttt{4\_reports/}，feedback 和 replay 写入 \texttt{5\_feedback/} 与 \texttt{5\_replay/}，HTML 报告写入 \texttt{6\_html/}，运行 manifest 写入 \texttt{7\_manifests/}。
...
\item
  聚合方面统计并生成报告。
\item
  构建 claim attribution、trace、feedback/replay 和 manifest。
\item
  可选导出 HTML 报告。
\end{enumerate}
...
当 manifest 或 analysis artifact 包含 \texttt{retrieval\_relevance} 标注时，评估模块计算 Recall@1、Recall@3、Recall@5、MRR、NDCG@3 和 NDCG@5。这些指标是信息检索评估中的常用排序与召回指标 \citep{manning2008introductionIr}。相关证据可以是 \texttt{region\_id}、\texttt{text\_block\_id}、\texttt{image\_id} 或 \texttt{evidence\_id}。
...
当前系统已经形成从原始商品目录到结构化 VOC 报告的端到端闭环。\texttt{run\_workflow.py} 能够将数据概览、标准化、索引构建和分析串联为单商品流程；\texttt{validate\_multimodal\_runtime.py} 能够检查运行时是否启用真实多模态链路；\texttt{export\_html\_report.py} 能够将分析产物导出为可审查页面；\texttt{evaluate\_manifests.py} 能够读取 run manifest 并计算评估指标。

这表明系统已经不再是若干独立脚本，而是一个以服务层、schema 和 artifact 为共同边界的分析框架。即使在 \texttt{-\/-no-llm} 离线模式下，系统仍能输出同构的方面、问题、检索和报告对象，从而支持调试和回归测试。
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}@{}}
\caption{当前代码基线下的系统验证结果。}\tabularnewline
\toprule\noalign{}
...
该机制使检索记录天然带有解释性。每个 retrieval 都可以回到 \texttt{source\_aspect\_id} 和 \texttt{source\_question\_id}，并且 \texttt{expected\_evidence\_routes} 指明该问题为什么需要文本证据、图像证据或二者结合。因此，问题规划不仅提升召回可控性，也为后续评估提供了 query-level 单元。

\subsection{多模态证据路线分析}
...
\texttt{ManifestEvaluationService} 已经支持两类评估。若运行产物包含人工检索标签，系统能够计算 Recall@K、MRR 和 NDCG；若没有人工标签，系统仍能计算结构化运行统计和 claim attribution 指标。后者包括 claim support rate、claim grounded rate、citation precision、citation contradiction rate 和 route contribution。
