...
- `6_experiments/`：实验矩阵输出、实时进度 dashboard 与快速测试输出。
- `runtime_logs/`：tmux、批处理和人工启动脚本的运行日志。
...
5. 实验矩阵汇总和 dashboard 落到 `6_experiments/`
6. 如果需要人工审核，再额外生成 `human_review.md` 这类面向人阅读的文件
...
- `6_experiments/` 中非 `current` 的快速测试目录可按需清理
- `runtime_logs/` 中的日志可随实验批次归档或清理
