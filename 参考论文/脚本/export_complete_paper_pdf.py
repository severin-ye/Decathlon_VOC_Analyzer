...
DEFAULT_INPUT = "脚本/outputs/中间文件/01_完整合并/CuraView_Complete_Paper.md"
DEFAULT_OUTPUT = "outputs/CuraView.pdf"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="输入 Markdown 文件名或路径，默认使用中间文件/01_完整合并 下的 CuraView_Complete_Paper.md",
    )
    parser.add_argument(
        "--output",
        default=DEFAULT_OUTPUT,
        help="输出 PDF 文件名或路径，默认写入 脚本/outputs/CuraView.pdf",
    )
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="curaview_pdf_") as temp_dir:
